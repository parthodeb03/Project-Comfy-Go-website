...
$stmt = $pdo->prepare("
    SELECT m.manager_ID, m.manager_name, m.manager_email, m.manager_mobile,
           ...
           h.hotel_location, h.hotel_rating, h.hotel_price, h.hotel_description
    FROM   Manager m
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        $new_hotel_desc     = trim($_POST['hotel_description'] ?? '');

        if (!$new_hotel_name || !$new_hotel_division || !$new_hotel_district || !$new_hotel_location) {
            $error = 'Please fill in all hotel fields.';
        } else {
            $upd = $pdo->prepare("UPDATE Hotels SET hotel_name=?, hotel_division=?, hotel_district=?, hotel_location=?, hotel_rating=?, hotel_price=?, hotel_description=? WHERE hotel_registration_number=?");
            $upd->execute([$new_hotel_name, $new_hotel_division, $new_hotel_district, $new_hotel_location, $new_hotel_rating, $new_hotel_price, $new_hotel_desc, $manager['hotel_registration_number']]);
        ...
        if ($booking_id) {
            $stmt = $pdo->prepare("UPDATE Booking SET booking_confirmation = 'Confirmed' WHERE booking_ID = ? AND hotel_registration_number = ?");
            $stmt->execute([$booking_id, $manager['hotel_registration_number']]);
        ...
        $booking_id = trim($_POST['booking_id'] ?? '');
        if ($booking_id) {
            $stmt = $pdo->prepare("UPDATE Booking SET booking_confirmation = 'Rejected' WHERE booking_ID = ? AND hotel_registration_number = ?");
            $stmt->execute([$booking_id, $manager['hotel_registration_number']]);
 ...
 $hbq = $pdo->prepare("
     ...
     ORDER  BY b.booking_date DESC
 ");
 $hbq->execute([$manager['hotel_registration_number']]);
 $hotel_bookings = $hbq->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div id="manager_content">
      <section class="m_section" id="overview">
        ...
        <div class="m_info_grid">
          ...
          <div class="m_info_card">
            ...
            <p class="m_info_value">৳<?= number_format($manager['hotel_price'] ?? 0) ?> <small style="font-weight:400;color:#888;">/ person / night</small></p>
          </div>
          <div class="m_info_card" style="grid-column:1/-1;">
            <p class="m_info_label">Description</p>
            <p class="m_info_value"><?= nl2br(htmlspecialchars($manager['hotel_description'] ?? '—')) ?></p>
          </div>
        </div>
      </section>
       ...
       <section class="m_section" id="bookings">
         ...
         <?php if (!empty($pending_bookings)): ?>
           <h3 class="m_sub_heading">Pending Approvals</h3>
                 ...
                 <tr>
                   ...
                   <td>
                     <form method="POST" action="manager.php" style="display:inline;">
                       ...
                       <button type="submit" class="m_btn_approve">Approve</button>
                     </form>
                     <form method="POST" action="manager.php" style="display:inline;">
                       <input type="hidden" name="action" value="reject_booking">
                       <input type="hidden" name="booking_id" value="<?= htmlspecialchars($b['booking_ID']) ?>">
                       <button type="submit" class="m_btn_reject" onclick="return confirm('Reject this booking?')">Reject</button>
                     </form>
                   </td>
                 </tr>
                 <?php endforeach; ?>
         ...
         <?php if (empty($hotel_bookings) && empty($pending_bookings)):?>
           <p class="m_empty">No bookings yet. Tourists will be able to book once your hotel details are filled in.</p>
         <?php else: ?>
           <h3 class="m_sub_heading" style="margin-top:28px;">Booking History</h3>
           <div class="m_table_wrap">
             <table class="m_table">
               <thead>
                 <tr>
      ...
      <section class="m_section" id="hotel_edit">
        ...
        <form method="POST" action="manager.php">
          ...
          <div class="m_field" style="grid-column:1/-1;">
            <label>Hotel Description</label>
            <textarea name="hotel_description" rows="4" placeholder="Describe your hotel — amenities, atmosphere, nearby attractions…" style="width:100%;padding:10px 14px;border:1px solid var(--border);border-radius:8px;font-family:inherit;font-size:14px;resize:vertical;"><?= htmlspecialchars($manager['hotel_description'] ?? '') ?></textarea>
          </div>
